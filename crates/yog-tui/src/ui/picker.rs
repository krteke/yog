...
pub trait DrawPicker {
    fn draw_picker(&mut self, area: Rect, picker: &FilePicker);
    fn render_entries(&mut self, area: Rect, picker: &FilePicker);
    fn render_footer_picker(&mut self, area: Rect, error: Option<&str>);
}

impl DrawPicker for Frame<'_> {
    fn draw_picker(&mut self, area: Rect, picker: &FilePicker) {
        ...
        self.render_entries(rows[1], picker);
        self.render_footer_picker(rows[2], picker.error());
    }
    ...
    fn render_entries(&mut self, area: Rect, picker: &FilePicker) {
        ...
        let lines = picker
            ...
            .collect::<Vec<_>>();
        self.render_widget(Paragraph::new(lines), area);
    }
